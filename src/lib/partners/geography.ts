...
const coveredCities = new Map(Object.values(REGIONS).flatMap((region) => region.cities.map((city, index) => [normalize(city), { region: region.slug, citySlug: region.citySlugs[index] }] as const)));
