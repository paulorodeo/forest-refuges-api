...
export function RegionalHub({ region, properties, posts }: { region: RegionConfig; properties: PropertyListResult; posts: BlogListResult }) {
  ...
  return <div className="min-h-screen bg-background"><SiteHeader /><SeoBreadcrumbs items={[{ name: "Início", href: "/" }, { name: "Regiões", href: "/regioes" }, { name: region.name }]} /><main>
    <header className="bg-forest py-16 text-forest-foreground"><div className="mx-auto max-w-6xl px-4"><p className="text-xs uppercase tracking-[0.18em] text-forest-foreground/70">Hub regional</p><h1 className="mt-3 text-4xl sm:text-5xl">{region.slug === "sao-paulo" ? "Chácaras, Refúgios e Casas de Campo perto de São Paulo" : `Imóveis na ${region.name}`}</h1><div className="mt-4 max-w-3xl space-y-3 text-forest-foreground/80">{region.intro.split("\n\n").map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></div></header>
    ...
    <section className="mx-auto max-w-6xl px-4 pb-10"><h2 className="text-2xl">Busque um imóvel nesta região</h2><p className="mt-2 mb-4 text-sm text-muted-foreground">Você pode alterar a cidade ou pesquisar por outro tipo de propriedade.</p><PropertySearch context={`Pesquisar em ${region.name}`} /></section>
    <section className="mx-auto max-w-6xl px-4 pb-12"><ListingView shell={false} title={`Propriedades na ${region.name}`} intro="Imóveis reais publicados no WordPress e filtrados pelas cidades desta região." items={properties.items} total={properties.total} page={properties.page} totalPages={properties.totalPages} paginationPath={`/regiao/${region.slug}`} unavailable={properties.unavailable} /></section>
